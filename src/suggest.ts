import type { AgentPattern, PatternSuggestion, SuggestPatternsInput } from "./types.js";

export function suggestPatterns(input: SuggestPatternsInput): PatternSuggestion[] {
  if (input.llm) {
...
function scorePattern(pattern: AgentPattern, context: string): PatternSuggestion {
...
function tokensFor(pattern: AgentPattern): string[] {
  return unique([
    pattern.id,
    pattern.title,
...
function containsAnyWord(context: string, text: string): boolean {
...
function sortSuggestions(suggestions: PatternSuggestion[]): PatternSuggestion[] {
