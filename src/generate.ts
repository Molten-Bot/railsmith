...
export function generateAgentsMd(config: GenerateConfig = {}): GenerateResult {
  ...
  const validSelections = selections.filter((selection, index) =>
    !hasErrors(validatePattern(selection.pattern, index))
  );
...
function renderScopedFile(scope: string, patterns: AgentPattern[], mode: OutputMode, blockName: string): string {
  const body = [
...
function renderProjectOverview(project: { name?: string; description?: string } | undefined, facts: RepoFacts, mode: OutputMode): string {
...
function renderSetup(facts: RepoFacts, mode: OutputMode): string {
  const items: string[] = [];
...
function renderTesting(facts: RepoFacts, mode: OutputMode): string {
...
function renderCodeStyle(facts: RepoFacts, mode: OutputMode): string {
  const items: string[] = [];
...
function renderArchitecture(patterns: AgentPattern[], scopedFiles: string[], mode: OutputMode): string {
  const items = [
...
function renderSecurity(mode: OutputMode): string {
  const items = [
...
function renderPullRequests(mode: OutputMode): string {
  const items = [
...
function renderCustomSection(input: SectionInput): string {
  const body = [
    input.body?.trim(),
...
function renderPatternSection(patterns: AgentPattern[], mode: OutputMode): string {
  if (patterns.length === 0) {
    return "";
  }
...
function renderPattern(pattern: AgentPattern, mode: OutputMode): string {
  const parts = [
    `### ${pattern.title}`,
...
function preferredScripts(scripts: Record<string, string>, names: string[]): string[] {
  return names.filter((name) => scripts[name] !== undefined);
}

function installCommand(packageManager: NonNullable<RepoFacts["packageManager"]>): string {
...
function scriptCommand(packageManager: NonNullable<RepoFacts["packageManager"]>, scriptName: string): string {
...
function normalizePatternMarkdown(markdown: string): string {
  return markdown
    .trim()
    .split(/\r?\n/)
...
function normalizeSelections(inputs: Array<AgentPattern | PatternSelection>): PatternSelection[] {
...
function groupScopedSelections(selections: PatternSelection[]): Map<string, PatternSelection[]> {
...
function createManifest(root: string, mode: OutputMode, files: GeneratedFile[], selections: PatternSelection[]): AgentsManifest {
  return {
    version: 1,
    root,
    mode,
...
function emptyRepoFacts(root: string): RepoFacts {
  return {
    root,
    scripts: {},
    dependencies: {},
